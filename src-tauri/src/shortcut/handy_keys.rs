...
impl HandyKeysState {
    ...
    /// Register every hotkey in a binding. Returns `Ok` if at least one
    /// hotkey registered (or the binding has none); only errors when every
    /// hotkey in the binding failed to register.
    pub fn register(&self, binding: &ShortcutBinding) -> Result<(), String> {
        super::aggregate_hotkey_results(binding.current_bindings.iter().map(|hk| {
            self.register_hotkey(&binding.id, hk).inspect_err(|e| {
                error!(
