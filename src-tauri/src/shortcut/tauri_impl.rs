...
pub fn register_hotkey(
    ...
    // Clone binding_id for use in the closure
...
/// Returns `Ok` if at least one hotkey registered (or the binding has none);
/// only errors when every hotkey in the binding failed to register, so a
/// single bad hotkey doesn't hide the others successfully registering.
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    super::aggregate_hotkey_results(binding.current_bindings.iter().map(|hk| {
        register_hotkey(app, &binding.id, hk).inspect_err(|e| {
            warn!(
