...
//!
//! This module is macOS-only; it is gated out entirely in `lib.rs`
//! (`#[cfg(target_os = "macos")] mod macos_input_source_guard;`), and its
//! only call site in `shortcut/handler.rs` is gated the same way.
...
/// including the initial snapshot — are dispatched onto the Tauri main
/// thread because IMK requires a CFRunLoop and
/// `TSMCurrentKeyboardInputSourceRefCreate` (called internally by TIS
...
pub fn arm(app: &AppHandle) {
    ...
    if dispatched.is_err() {
        // The outer dispatch itself never ran (e.g. main thread/event loop
        // already gone), so neither of the ARMED resets above will fire.
        // Reset here or the flag would stay true for the rest of the
        // process lifetime, permanently disabling the guard.
        ARMED.store(false, Ordering::Release);
    }
}
...
mod tests {
    ...
    #[test]
    fn hotkey_contains_fn_matches_case_insensitively_and_with_modifiers() {
        assert!(hotkey_contains_fn("fn"));
        assert!(hotkey_contains_fn("Fn"));
        assert!(hotkey_contains_fn("ctrl+fn"));
        assert!(hotkey_contains_fn("fn+ctrl"));
        assert!(hotkey_contains_fn(" ctrl + FN "));
    }

    #[test]
    fn hotkey_contains_fn_rejects_non_fn_hotkeys() {
        assert!(!hotkey_contains_fn("ctrl+shift+f"));
        assert!(!hotkey_contains_fn("f13"));
        assert!(!hotkey_contains_fn(""));
    }
}
