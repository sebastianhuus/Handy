...
/// list/selection/loaded state, microphone selection, language, settings).
...
fn build_menu(app: &AppHandle, inputs: &MenuInputs) -> tauri::Result<(Menu<tauri::Wry>, String)> {
    ...
    let menu = if inputs.busy {
        ...
        for device in &inputs.mic_devices {
            let item_id = format!("mic_default_select:{}", device);
        ...
        let clamshell_mic_submenu = {
            ...
            for device in &inputs.mic_devices {
                let item_id = format!("mic_clamshell_select:{}", device);
                let item = CheckMenuItem::with_id(
                    app,
                    &item_id,
                    device,
                    true,
                    inputs.clamshell_microphone.as_deref() == Some(device.as_str()),
                    None::<&str>,
                )?;
                submenu.append(&item)?;
            }
        ...
        #[cfg(target_os = "macos")]
        menu.append(&clamshell_mic_submenu)?;
        menu.append(&separator()?)?;
