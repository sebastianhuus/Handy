...
async setNoiseSuppression(enabled: boolean) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("set_noise_suppression", { enabled }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async setConvertNumberWords(enabled: boolean) : Promise<void> {
    await TAURI_INVOKE("set_convert_number_words", { enabled });
},
 ...
 */
settings_schema_version?: number; bindings: Partial<{ [key in string]: ShortcutBinding }>; audio_feedback: boolean; audio_feedback_volume?: number; sound_theme?: SoundTheme; start_hidden?: boolean; autostart_enabled?: boolean; update_checks_enabled?: boolean; show_whats_new_on_update?: boolean;
/**
